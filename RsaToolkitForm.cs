...
namespace Sha_and_RSA
{
    public partial class RsaToolkitForm : Form
    {
        ...
        {
            if (tabControlRsa.TabPages.ContainsKey(tabKey))
            {
                tabControlRsa.SelectTab(tabKey);
            }
        }
        public void SelectTab(int tabIndex)
        {
            // Geçersiz bir indeks girilme ihtimaline karşı kontrol eklemek her zaman iyidir.
            if (tabIndex >= 0 && tabIndex < tabControlRsa.TabCount)
            {
                tabControlRsa.SelectedIndex = tabIndex;
            }
        }
